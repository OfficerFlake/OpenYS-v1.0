...
namespace OpenYS
{
    ...
    {        
        ...
        public static bool ShutdownOnNextRestart = false;
        ...
        public static TimeSpan GetTimeLeftUntilReset()
        {
            if (Settings.Server.RestartTimer > 0)
                return new TimeSpan(0, Settings.Server.RestartTimer, 0) - (DateTime.Now - Time_ResetTimerStarted);
            else
        ...
        public static bool PollResetTimer()
        {
            ...
            {
                #region ResetTimer
                TimeSpan TimeRemaining = GetTimeLeftUntilReset();
        ...
        {
            ShutdownOnNextRestart = true;
            Clients.AllClients.SendMessage("&6*** Server shutting down! ***");
            Thread.Sleep(1.AsSeconds()); //sleep for 1 seconds.
            Clients.AllClients.SendMessage("&6*** Server shutting down! ***");
            Thread.Sleep(1.AsSeconds()); //sleep for 1 seconds.
            Clients.AllClients.SendMessage("&6*** Server shutting down! ***");
