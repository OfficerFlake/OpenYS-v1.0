...
namespace OpenYS
{
    public static partial class Log
    {
        ...
        /// <returns>True if log successful</returns>
        public static bool Warning(string Out, params object[] args)
        {
            if (args == null) args = new object[0];
            if (args.Length > 0)
            {
            ...
            lock (Threads.GenericThreadSafeLock)
            {
                ...
                string LogFile = "./Logs/WarningLogs/" + OYS_DateTime.ToOYSLongDateTime(TimeStarted).ReplaceAll("/", "_").ReplaceAll(" ", "_").ReplaceAll(":", "_") + ".html";
                Files.PrepareHTMLLog(LogFile);
                List<string> MessagesOut = new List<string>();
        ...
        /// <param name="Out">String to output</param>
        /// <returns>True if log successful</returns>
        public static bool Chat(string Input, params object[] args)
        {
            if (args == null) args = new object[0];
            if (args.Length > 0)
            {
            ...
            lock (Threads.GenericThreadSafeLock)
            {
                ...
                string LogFile = "./Logs/ChatLogs/" + OYS_DateTime.ToOYSLongDateTime(TimeStarted).Split(' ')[0].ReplaceAll("/", "_") + ".html";
                Files.PrepareHTMLLog(LogFile);
                String[] MessageOut = { String.Format("{0}: {1}", OYS_DateTime.ToOYSLongDateTime(DateTime.Now), Input) };
                Files.FileHTMLAppend(LogFile, MessageOut);
            }
